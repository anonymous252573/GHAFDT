...
mice_data_root = r'C:\....\{}_hippo\all_data_interp'.format(prot)
condis_mice = ['CN', 'AD']   
for condi in condis_mice:
    save_folder_orig = os.path.join(mice_data_root, '{}_orig_shape_space'.format(condi))
    os.makedirs(save_folder_orig, exist_ok=True)
    save_folder_tangent_data = os.path.join(mice_data_root, '{}_tangent_data'.format(condi))
    os.makedirs(save_folder_tangent_data, exist_ok=True)

    if condi == 'CN':
        save_folder_karcher = os.path.join(mice_data_root, '{}_Karcher_mean'.format(condi))
        os.makedirs(save_folder_karcher, exist_ok=True)
    
    # *** count all available time points***
    folder_path = os.path.join(mice_data_root, condi)
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith('.txt') and filename.startswith('matched_'):
            ...
            if '_' in base:
                subject_id, timepoint = base.split('_')
                subject_timepoints[subject_id].append(timepoint)
                unique_timepoints.add(timepoint)

    # Count subjects without a '2M' timepoint or with either missing 2M, 4M or 12M
    subjects_without_2M = [sid for sid, times in subject_timepoints.items() if '2M' not in times]
    subjects_with_2M = [sid for sid, times in subject_timepoints.items() if '2M' in times]
    subjects_with_missing_tp = [sid for sid, times in subject_timepoints.items() if '2M' not in times or '4M' not in times or '12M' not in times]
    subjects_without_missing_tp = [sid for sid, times in subject_timepoints.items() if '2M' in times and '4M' in times and '12M' in times]

    if subjects_with_missing_tp:
        print(f" These ids {subjects_with_missing_tp} are missing time points in {condi} folder\n")
        # print(f"Total number of subjects with missing tp': {len(subjects_with_missing_tp)}\n")
    if subjects_without_2M:
        print(f" These ids {subjects_without_2M} are missing 2M (bl) scan in {condi} folder\n")
    ...
    if condi == 'CN':
        ...
        for subject_id in subjects_with_2M: 
    ...
    mean_path = os.path.join(mice_data_root, '{}_Karcher_mean'.format(condis_mice[0]), '{}_Karcher_mean_shape.pkl'.format(condis_mice[0]) )
    with open(mean_path, 'rb') as f:
        CN_temporal_means = pickle.load(f)
    baseline_mean = CN_temporal_means['2M']
...
print("======================= MICE COMPLETED ============================")

h_dataset = 'ADNI'
human_data_root = r'C:\....\{}_DG'.format(h_dataset)
condis_human = ['CN_val', 'CN_test', 'AD_val', 'AD_test'] 

for condi in condis_human:
    save_folder_orig = os.path.join(human_data_root, '{}_orig_shape_space'.format(condi))
    os.makedirs(save_folder_orig, exist_ok=True)
    save_folder_tangent_data = os.path.join(human_data_root, '{}_tangent_data'.format(condi))
    os.makedirs(save_folder_tangent_data, exist_ok=True)

    if condi == 'CN_val':
        save_folder_karcher = os.path.join(human_data_root, '{}_Karcher_mean'.format(condi))
        os.makedirs(save_folder_karcher, exist_ok=True)

    folder_path = os.path.join(human_data_root, condi)
    subject_timepoints = defaultdict(list)
    unique_timepoints = set()
    ...
    # **** We compute temporal Karcher mean for the Normal subjects (just like mice) *****
    if condi == 'CN_val':
        temporal_shapes = defaultdict(list)
    ...
    # we use the baseline of CN as the common tangent to transport all vectors 
    mean_path = os.path.join(human_data_root, '{}_Karcher_mean'.format(condis_human[0]), '{}_Karcher_mean_shape.pkl'.format(condis_human[0]) )
    with open(mean_path, 'rb') as f:
        CN_temporal_means = pickle.load(f)
    ...
    for subject_id in subjects_with_bl:
        ...
        # Then save the data for this subject
        timepoint_str = '_'.join(sorted(str(k) for k in orig_shape_dict.keys()))
        save_filename = f"{subject_id}_{timepoint_str}.pkl"
