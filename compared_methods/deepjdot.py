...
# the gamma from the paper (check algorithm 1 of the paper)
# the OT solver here which even allows flow of gradients

#The official Keras implementation for DeepJDOT relies on the POT (Python Optimal Transport) 
# library, which uses Sinkhorn or similar regularized OT solvers by default
def sinkhorn(a, b, C, epsilon=0.1, n_iters=20):
    ...
    for _ in range(n_iters):
        Kv = K @ v
        Kv = torch.clamp(Kv, min=1e-8)
        u = a / Kv

        KTu = K.t() @ u
        KTu = torch.clamp(KTu, min=1e-8)
        v = b / KTu

    gamma = torch.diag(u) @ K @ torch.diag(v)
    return gamma

def deepjdot_loss(f_s, y_s, f_t, y_t, alpha=0.1, epsilon=0.1):
    f_s = F.normalize(f_s, dim=1)
    f_t = F.normalize(f_t, dim=1)

    Ns, Nt = f_s.size(0), f_t.size(0)

    # Uniform marginals
    a = torch.full((Ns,), 1.0 / Ns, device=f_s.device)
    b = torch.full((Nt,), 1.0 / Nt, device=f_t.device)

    # Feature cost
    C_feat = pairwise_squared_distances(f_s, f_t)

     # Label cost (0 if same class, 1 otherwise)
    C_label = (y_s.unsqueeze(1) != y_t.unsqueeze(0)).float()  # this is correct for broadcast comparison

    # Joint cost    
    C = C_feat + alpha * C_label
    C = C - C.min()  # shift for numerical stability

    # Transport plan
    gamma = sinkhorn(a, b, C, epsilon=epsilon)

    # OT loss
    loss = torch.sum(gamma * C)
    return loss

def train_one_epoch(model, source_loader, target_loader, optimizer, criterion):
    model.train()
    ...
    for x_s, y_s, mask_s in source_loader:
        ...
        loss_ot = deepjdot_loss(features_s, y_s,features_t, y_t, alpha=1.0, epsilon=0.05)
        loss = loss_ce + loss_ot

        loss.backward()  # this is the step 2 of the algorithm where we fix gamma
        optimizer.step()
