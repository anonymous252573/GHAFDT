...
def covariance(features, eps=1e-5):
    N = features.size(0)
    mean = features.mean(dim=0, keepdim=True)
    centered = features - mean
    cov = (centered.T @ centered) / (N - 1)
    cov += eps * torch.eye(features.size(1), device=features.device)
    return cov

def class_conditional_coral(f_s, y_s, f_t, y_t, num_classes, eps=1e-5):
    loss = 0.0
    valid_classes = 0
    ...
    for c in range(num_classes):
        ...
        if idx_s.sum() < 2 or idx_t.sum() < 2:
            continue

        # Compute covariance
        cov_s = covariance(f_s[idx_s])
        cov_t = covariance(f_t[idx_t])

        # Weight proportional to class frequency
        class_weight = total_samples_per_class[c] / total_samples_sum

        loss += class_weight * torch.norm(cov_s - cov_t, p='fro') ** 2
        valid_classes += 1
...
def train_one_epoch(model, source_loader, target_loader, optimizer, criterion, lambda_coral=0.005):
    model.train()
    ...
    for x_s, y_s, mask_s in source_loader:
        ...
        loss_coral = class_conditional_coral(features_s, y_s, features_t, y_t, num_classes=2)
        lambda_coral = min(0.005, epoch+1/25) 
