...
def domain_adaptive_supcon_loss(f_s, y_s, f_t, y_t, temperature=0.1):
    ...
    for i in range(f_s.size(0)):
        ...
        # Positives: target samples, same class
        pos_mask = (y_t == label)
        if pos_mask.sum() == 0:
            continue
        ...
        # Negatives: all other target samples
        negatives = f_t

        # Similarities
        pos_sim = torch.matmul(positives, anchor)
        neg_sim = torch.matmul(negatives, anchor)

        numerator = torch.exp(pos_sim / temperature).sum()
        denominator = torch.exp(neg_sim / temperature).sum()
...
def train_one_epoch(model, source_loader, target_loader, optimizer, criterion, epoch, margin=1.0):
    model.train()
    ...
    for x_s, y_s, mask_s in source_loader:
        ...
        # CE loss (source only, as before)
        loss_ce = criterion(logits_s, y_s)

        # Contrastive DA loss
        temperature = 0.07   # tried different temperature and this gives the best on training without overfitting
        loss_contrastive = domain_adaptive_supcon_loss(features_s, y_s, features_t, y_t, temperature=temperature)

        lambda_c = min(0.05, (epoch + 1) / 25)

        loss = loss_ce + lambda_c * loss_contrastive
