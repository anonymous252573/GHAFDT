...
def gaussian_kernel(x, y, sigma=1.0):
    ...
    kernel = torch.exp(-((x - y) ** 2).sum(2) / (2 * sigma ** 2))
    return kernel

def mmd_rbf(f_s, f_t, sigmas=(1, 2, 5, 10)):
    loss = 0.0
    for sigma in sigmas:
...
def class_conditional_mmd(f_s, y_s, f_t, y_t, num_classes):
    loss = 0.0
    for c in range(num_classes):
        ...
        if idx_s.sum() == 0 or idx_t.sum() == 0:
            continue
        loss += mmd_rbf(f_s[idx_s], f_t[idx_t])
    return loss

def train_one_epoch(model, source_loader, target_loader, optimizer, criterion, lambda_mmd=0.5):
    model.train()
    ...
    for x_s, y_s, mask_s in source_loader:
        ...
        # CC-MMD between source and target
        loss_mmd = class_conditional_mmd(features_s, y_s, features_t, y_t, num_classes=2)
