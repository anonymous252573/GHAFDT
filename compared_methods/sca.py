...
# domain alginment loss (we use MMD)
...
def mmd_loss(f_s, f_t, sigma=1.0):
    K_ss = gaussian_kernel(f_s, f_s, sigma)
    K_tt = gaussian_kernel(f_t, f_t, sigma)
    K_st = gaussian_kernel(f_s, f_t, sigma)
    return K_ss.mean() + K_tt.mean() - 2 * K_st.mean()

def domain_adaptive_triplet_loss(f_s, y_s, f_t, y_t, margin=1.0):
    loss = 0.0
    valid_triplets = 0

    for i in range(f_s.size(0)):
        anchor = f_s[i]
        anchor_label = y_s[i]

        # Positives: target samples with same class
        pos_mask = (y_t == anchor_label)
        if pos_mask.sum() == 0:
            continue

        positives = f_t[pos_mask]

        # Negatives: source + target samples with different class
        neg_mask_s = (y_s != anchor_label)
        neg_mask_t = (y_t != anchor_label)

        negatives = torch.cat([f_s[neg_mask_s], f_t[neg_mask_t]], dim=0)

        if negatives.size(0) == 0:
            continue

        # Compute distances
        d_ap = torch.cdist(anchor.unsqueeze(0), positives).squeeze(0)
        d_an = torch.cdist(anchor.unsqueeze(0), negatives).squeeze(0)

        # Hardest positive & negative
        hardest_pos = d_ap.max()
...
def train_one_epoch(model, source_loader, target_loader, optimizer, criterion, epoch, lambda_triplet=0.1, lambda_domain=0.1, margin=1.0):
    model.train()
    ...
    for x_s, y_s, mask_s in source_loader:
        ...
        # Forward
        logits_s, f_s = model(x_s, mask_s)
        logits_t, f_t = model(x_t, mask_t)
        ...
        #  Triplet loss
        loss_triplet = domain_adaptive_triplet_loss(f_s, y_s,f_t, y_t,margin=margin)
        loss_domain = mmd_loss(f_s, f_t)

        # Ramp-up
        lambda_triplet = min(0.1, (epoch + 1) / 40)
        lambda_domain = min(0.1, (epoch + 1) / 40)
...
for epoch in range(epochs):
    # Train and evaluate
    train_loss, train_acc, train_auc, train_preci, train_rec = train_one_epoch(model, train_loader, val_loader, optimizer, criterion, epoch=epoch)
